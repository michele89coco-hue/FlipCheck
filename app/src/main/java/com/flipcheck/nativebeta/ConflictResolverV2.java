...
final class ConflictResolverV2 {
    ...
    static List<Conflict> resolve(ImmutableEvidenceLedgerV2 ledger,DomainProfileRouterV2.Profile profile){List<Conflict>out=new ArrayList<>();String[]fields={"manufacturer","productLine","setName","cardName","athlete","collectorNumber","physicalCardNumber","productReleaseYear","model","edition"};
        for(String field:fields){List<EvidenceAtom>facts=ledger.current(field);for(int i=0;i<facts.size();i++)for(int j=i+1;j<facts.size();j++){EvidenceAtom a=facts.get(i),b=facts.get(j);if(!eligible(a)||!eligible(b)||a.epistemicLevel==EvidenceAtom.EpistemicLevel.INFERRED||b.epistemicLevel==EvidenceAtom.EpistemicLevel.INFERRED||TypedFieldNormalizerV2.ambiguous(a.normalizedValue)||TypedFieldNormalizerV2.ambiguous(b.normalizedValue)||TypedFieldNormalizerV2.equivalent(field,a.normalizedValue,b.normalizedValue))continue;
