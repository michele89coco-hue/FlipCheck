...
final class ConflictResolverV2 {
    ...
    static List<Conflict> resolve(ImmutableEvidenceLedgerV2 ledger,DomainProfileRouterV2.Profile profile){List<Conflict>out=new ArrayList<>();String[]fields={"manufacturer","productLine","setName","slabSetName","slabYear","cardName","athlete","collectorNumber","physicalCardNumber","productReleaseYear","model","edition"};
        for(String field:fields){if(!identityField(field,profile))continue;List<EvidenceAtom>facts=ledger.current(field);for(int i=0;i<facts.size();i++)for(int j=i+1;j<facts.size();j++){EvidenceAtom a=facts.get(i),b=facts.get(j);SemanticRelationV3.Relation relation=SemanticRelationV3.relate(field.equals("slabSetName")?"setName":field.equals("slabYear")?"productReleaseYear":field,a.normalizedValue,b.normalizedValue);if(!eligible(a)||!eligible(b)||a.epistemicLevel==EvidenceAtom.EpistemicLevel.INFERRED||b.epistemicLevel==EvidenceAtom.EpistemicLevel.INFERRED||TypedFieldNormalizerV2.ambiguous(a.normalizedValue)||TypedFieldNormalizerV2.ambiguous(b.normalizedValue)||SemanticRelationV3.compatible(relation)||relation==SemanticRelationV3.Relation.AMBIGUOUS)continue;
