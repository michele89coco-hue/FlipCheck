...
final class EvidenceLedger {
    static final String DIRECT_PHOTO_OBSERVATION="DIRECT_PHOTO_OBSERVATION";
    static final String PRIMARY_VISION="PRIMARY_VISION", FOCUSED_VISION="FOCUSED_VISION";
    static final String LOCAL_OCR="LOCAL_OCR", WEB_CATALOG="WEB_CATALOG";
    static final String WEB_MARKET="MARKETPLACE", USER_HINT="USER_HINT", INFERRED="INFERRED";
    static final String PHOTO=PRIMARY_VISION;
    ...
    static void ingestPhotoObservation(Models.Identification id, JSONObject photoIdentity) {
        ...
        if(structured!=null)for(int i=0;i<structured.length();i++){
            ...
            add(id,new Models.EvidenceFact(key,value,PRIMARY_VISION,clean(x.optString("evidence_type","vision")),
        ...
        for(String raw:id.photoIdentityFields){String k=key(raw),v=value(raw);if(k.isEmpty()||unresolved(v))continue;
            ...
            add(id,new Models.EvidenceFact(k,v,PRIMARY_VISION,"vision_structured_field",id.photoIdentityConfidence,-1,
    ...
    static void ingestCompactPhotoObservation(Models.Identification id, JSONArray facts) {
        ...
        for(int i=0;i<facts.length();i++){
            ...
            add(id,new Models.EvidenceFact(k,v,PRIMARY_VISION,"primary_vision_observation",
    ...
    static void ingestFocusedObservation(Models.Identification id,JSONArray facts){if(id==null||facts==null)return;
        for(int i=0;i<facts.length();i++){JSONObject x=facts.optJSONObject(i);if(x==null)continue;
            String k=key(x.optString("key","")),v=clean(x.optString("value",""));if(k.isEmpty()||unresolved(v))continue;
            add(id,new Models.EvidenceFact(k,v,FOCUSED_VISION,clean(x.optString("evidence_type","focused_vision")),
                    x.optInt("confidence",0),x.optInt("image_index",-1),clean(x.optString("side","")),
                    clean(x.optString("location","")),clean(x.optString("semantic_role","")),""));}}
    ...
    static void addPhotoFact(Models.Identification id,String key,String value,String evidenceType,
        ...
        String origin=clean(evidenceType).toLowerCase(Locale.ROOT).contains("focused")?FOCUSED_VISION:PRIMARY_VISION;
        add(id,new Models.EvidenceFact(key(key),clean(value),origin,evidenceType,confidence,imageIndex,
    ...
    static Models.EvidenceFact bestPhotoFact(Models.Identification id,String...keys){
        ...
        for(Models.EvidenceFact f:id.evidenceLedger)if(f!=null&&isPixelOrigin(f.origin)&&matches(f.key,keys)
    ...
    static String debug(Models.EvidenceFact f){if(f==null)return "";return f.key+"="+f.value
            ...
            +", semantic_role="+f.semanticRole+", text_scope="+f.textScope+", pipeline_stage="+f.pipelineStage+(f.sourceUrl.isEmpty()?"":", source="+f.sourceUrl)+"]";}
    ...
    private static String valueFrom(Models.Identification id,boolean photoOnly,String...keys){
        ...
        for(Models.EvidenceFact f:id.evidenceLedger){if(f==null||!matches(f.key,keys)||unresolved(f.value))continue;
            if(photoOnly&&!isPixelOrigin(f.origin))continue;
            if(!photoOnly&&!isPixelOrigin(f.origin)&&!USER_HINT.equals(f.origin))continue;
    ...
    static boolean isPixelOrigin(String origin){return PRIMARY_VISION.equals(origin)||FOCUSED_VISION.equals(origin)
            ||DIRECT_PHOTO_OBSERVATION.equals(origin)||"photo".equalsIgnoreCase(origin);}
