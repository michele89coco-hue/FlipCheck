...
- Local bundled OCR supports Japanese script selection and retains Latin by default. Dense small print first locates compact sideways light text by local contrast and reads up to six regions together, in original and inverted form. It also retains inverted grayscale edge passes, capped at eight passes and the existing recovery time budget. Coordinates remain relative to the original photograph. No serial characters are invented or repaired by lookup.
...
The Android gate tests the real Boniface photograph, requiring local OCR itself to return 2/5 at its original coordinates. It also tests another serial at a different position, Japanese text with the bundled model, structured certificate page extraction, background completion, navigation and photo selection. Browser production replays enforce the same asset whitelist as the Android host. Build success alone is not evidence that this gate passed.
