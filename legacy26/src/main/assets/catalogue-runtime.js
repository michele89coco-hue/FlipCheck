...
async function compareCore193(l,entries,ctx,pages){
 const candidates=E193.candidateGroups(l,entries).filter(g=>g.score>=60&&g.entry.matches.includes('subject')&&g.entry.matches.includes('number')).slice(0,3),refs=[];
 for(const g of candidates){
  ...
  const url=image?.image_url||image?.url;if(!url||!l.attempt('core-image:'+url))continue;
