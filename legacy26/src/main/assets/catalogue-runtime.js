...
async function searchCatalogue193(l,ctx,mode,pages){
 ...
 const urls=[...new Set([...checklist.slice(0,1).map(s=>s.url),...(Array.isArray(reply.entries)?reply.entries:[]).map(e=>e.source_url),...discovered.map(s=>s.url)])].filter(url=>E193.sourceTrusted(url,l.domain)||l.domain==='generic'&&/^https:\/\//.test(url)).slice(0,4);
 for(const url of urls){if(!l.attempt('get:'+url))continue;try{const page=await directCall165('page',{url,terms:[E193.keyValues(l).subject,...E193.keyValues(l).numbers,...E193.keyValues(l).colors,'Parallels','Print Run'].filter(Boolean)},ctx,8500);if(page.status===200&&C193.usablePage200({...page,url:page.url||url}))all.push({...page,url:page.url||url});else l.record('catalogue_page_unusable',{url,status:page.status,reason:'empty_or_unavailable'});}catch(error){guard164(ctx);}}
...
async function detailRead193(l,ctx,requests,entries=[]){
 ...
 const contextPictures=[];for(const req of active.filter(r=>r.field==='serial'||r.reason==='catalogue_number_conflict'||r.reason==='code_preflight')){const region=(l.base.object_regions||[]).find(r=>r.image_index===req.image_index);contextPictures.push(await visualPhoto164({object_region:region?{...region,certain:true}:{image_index:req.image_index,certain:false}}));}
 const content=[{type:'input_text',text:'Rileggi esclusivamente questi dettagli dell’ORIGINALE. Le letture alternative sono ipotesi da verificare nei pixel: non scegliere quella che sembra una carta nota. Per serial leggi numeratore, barra e denominatore; 215 non equivale a 2/5 senza vedere la barra. Per collector_number conserva prefissi, non completare caratteri illeggibili. Per stamp/shadow/rarity_symbol rispondi present/absent/unclear; assente richiede la zona visibile. Per finish usa normal/holo/reverse/reflective/unclear. Per language leggi il testo strutturale e usa il codice della lingua solo se chiaro; non la lingua dell’app. evidence_found=true solo quando il dettaglio richiesto è stato verificato sulla foto originale indicata; vale anche per un’assenza verificata in una zona leggibile. Un dubbio resta uncertain con evidence_found=false. Il pannello di contesto stabilisce il verso di lettura: controlla numeri rovesciati e non confermare la precedente trascrizione per inerzia. Nessuna ricerca. Richieste: '+JSON.stringify(active)}];
...
async function compareCore193(l,entries,ctx,pages){
 ...
 for(const g of candidates){
  ...
  const url=image?.image_url||image?.url||cert?.image_url||e.image_url;if(!url||!l.attempt('core-image:'+url))continue;
...
async function resolveCatalogue193(base,ctx){
 ...
 try{
  ...
  if(['onepiece','tcg'].includes(l.domain)){const read=l.active('collector_number').find(a=>a.region&&a.region.width<.12&&(a.reported_field==='set_code'||l.active('collector_number').some(b=>!E193.numbersMatch(a.value,b.value))));if(read){const req={key:'code-preflight:'+read.image_index,field:'collector_number',reason:'code_preflight',region:read.region,image_index:read.image_index,rotation:0,readings:[]};await detailRead193(l,ctx,[req],entries);}}
