...
async function searchCatalogue193(l,ctx,mode,pages){
 ...
 const raw=collectRawWebResults(response),sources=enrichSources(collectSources(response),raw);let reply={entries:[]};try{reply=parseResponseJSON(response);}catch(error){l.record('catalogue_response_incomplete',{reason:responseReason166(error)});}
 const discovered=[...raw,...sources].filter((s,i,a)=>s.url&&a.findIndex(x=>x.url===s.url)===i),all=[...pages];
