...
Sportive: squadra e nazionalità sono team, non subset. subset è solo il nome stampato della sottoserie/inserto. Le frasi di licenza non sono product. Sportive: registra il badge RC/Rookie visibile come text e rookie=present, senza dedurlo dalla stagione. L'assenza non prova che la carta non sia rookie. Leggi atleta, produttore, prodotto, sottoserie, numero completo, stagione del prodotto e seriale a/b se presente in QUALSIASI foto, anche verticale. Confezioni: product, stagione, codice prodotto, numero di bustine/carte e promessa integrale con il denominatore (ogni N scatole). I giocatori sulla scatola non sono il prodotto.
...
async function inspectSurfaces199(l,result,ctx){
 ...
 if(result.variant_resolution.pending.includes('serial')||l.pick('serial')||l.pick('serial_presence')||!candidates.some(v=>v.print_run)||!candidates.some(v=>v.unnumbered||v.base_printing)||validImageCount()<2)return;
...
async function compareVariants193(l,result,ctx,pages,watermarkRetry=false){
 ...
 for(const variant of candidates){
  ...
  const url=linked?.url||linked?.image_url;if(!url)continue;const cached=ctx.variantImages201.get(url);if(cached){refs.push({id:'variant'+refs.length,variant,source:page.url,image_data:cached});continue;}if(!l.attempt('variant-image:'+url))continue;
...
async function resolveCatalogue193(base,ctx){
 ...
 try{
  ...
  if(l.domain==='pokemon'&&!lensReread)await readPokemonBands202(l,ctx);
