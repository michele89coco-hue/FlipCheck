...
async function readPhotoOcr174(base,ctx){
 const script=/^(?:japanese|giapponese|ja|jp)$/i.test(base.pokemon_printing?.language||base.language||'')?'japanese':'latin';
 if(ctx.photoOcr&&!(script==='japanese'&&ctx.ocrScript205==='latin'))return;const previous=ctx.photoOcr||[];ctx.photoOcr=[];ctx.ocrScript205=script;
 let available=false;try{available=window.FlipCheckGoogle?.ocrAvailable?.()===true;}catch(_){}
 if(!available)return;
 ...
 if(previous.length)ctx.photoOcr=previous.concat(ctx.photoOcr);
}
...
renderLiveCost=function(){priorLiveCost164();if(!scan164||!currentScan)return;const g=scan164.budget.entries.filter(e=>['visual','lens','ximilar'].includes(e.kind));if(!g.length)return;const el=$('liveCost');el.innerHTML=el.innerHTML.replace('Costo di questa analisi finora','Costo OpenAI da usage');const p=document.createElement('p');p.className='note';p.textContent='Totale API stimato, incluse ricerche immagini: $'+scan164.budget.spent().toFixed(4)+' · Ricerche immagini: '+g.length+' tentativo · eventuali addebiti incerti restano conteggiati nel limite.';el.append(p);};
$('marketBtn').onclick=async()=>{if(photoBusy||apiBusy)return;if(!scan164)scan164=newContext164();scan164.phase='market';scan164.budget.deadline=Date.now()+60000;const saved=ident,savedTrial=JSON.parse(JSON.stringify(trial)),calls=scan164.calls.length;try{await priorMarket164();}finally{if(V164.ready(saved)&&!V164.ready(ident))ident=saved;scan164.comparablesState=scan164.cardMarket?.state|| (scan164.state==='budget_exhausted'?'budget_exhausted':$('resultPanel').textContent.includes('DATI INSUFFICIENTI')?'unavailable':'requested');if(scan164.comparablesState==='budget_exhausted'&&scan164.calls.length===calls){trial=savedTrial;saveTrial();persistCompletedScan(saved.normalized_query||saved.model,'budget_exhausted');status('Identità conservata. Il budget rimasto non basta per la ricerca mercato.','warn');}scan164.phase='identity';scan164.state=scan164.identityState|| (V164.ready(ident)?'confirmed':'unidentified');renderLiveCost();}};
...
diagnostic26=function(){const d=priorDiagnostic164();let nativePhotoPicker=null;try{nativePhotoPicker=JSON.parse(window.FlipCheckHost?.photoPickerInfo?.()||'null');}catch(_){}return {...d,nativePhotoPicker,schema:'flipcheck-v0262-evidence-14',
 ...
 engine:scan164.engineSnapshot||null,stateRevision:scan164.catalogueState?.revision||0,jobStatus:scan164.jobStatus||null,catalogueRequests:scan164.catalogueRequests||[],catalogueCoverage:scan164.catalogueCoverage||null,scanId:scan164.id,testMode:scan164.mode,featureEnabled:visualConfig164().enabled,state:scan164.state,provider:scan164.provider,comparablesState:scan164.comparablesState||'not_requested',queries:scan164.queries,calls:scan164.calls,closures:scan164.closures,recoveries:scan164.recoveries,
 ximilar:scan164.ximilar,marketBudget:scan164.marketBudget,pokemonPanels:scan164.pokemonPanels,evidenceWorkspace:{...evidenceInfo192(),prepared_image_hits:scan164.imageCacheHits||0},localPrinting:scan164.localPrinting,localReferenceComparisons:scan164.localReferenceComparisons,priorityClosure:scan164.priorityClosure,evidenceTransitions:scan164.evidenceTransitions,dateVerification:scan164.dateVerification,route:scan164.route,certificateLookup:scan164.certificateLookup,catalogueRoute:scan164.catalogueRoute,catalogueFilter:scan164.catalogueFilter,catalogueFallback:scan164.catalogueFallback,slabVerification:scan164.slabVerification,specificationVerification:scan164.specificationVerification,photoEvidence:scan164.photoEvidence,cardKeyVerification:scan164.cardKeyVerification,configurationReread:scan164.configurationReread,textReferences:scan164.textReferences,photoOcr:scan164.photoOcr,evidenceFusion:scan164.evidenceFusion,focusedReview:scan164.focusedReview,fieldRepair:scan164.fieldRepair,comparisonPlanning:scan164.comparisonPlanning,excludedReferences:scan164.excludedReferences,localOcr:scan164.localOcr,retainedReferences:scan164.retainedReferences,detailReread:scan164.detailReread,secondQuery:scan164.secondQuery,deferredComparison:scan164.deferredComparison,continuationBudget:scan164.continuationBudget,referenceCompletion:scan164.referenceCompletion,initialImagePreparations:scan164.initialImagePreparations,imagePreparation:scan164.imagePreparation,imagePreparations:scan164.imagePreparations,comparisons:scan164.comparisons,printingRecovery:scan164.printingRecovery,coreIdentityState:scan164.coreIdentityState,identityState:scan164.identityState,catalogueRetrieval:scan164.catalogueRetrieval,comparison:scan164.comparison,budget:{targetUsd:scan164.budget.targetUsd,completionToleranceUsd:scan164.budget.completionToleranceUsd,maxUsd:scan164.budget.maxUsd,spentOrReservedUsd:scan164.budget.spent(),entries:scan164.budget.entries,visionCalls:scan164.budget.visionCalls,maxVisionCalls:4,estimated:true,includesIdentificationAndMarket:true},
 costNote:'OpenAI usage follows configured v26 rates; Google, SearchApi and Ximilar are estimated separately. Failed/time-out requests retain their reservation because billing may apply.'}: {state:active164()?'not_requested':'not_configured'}};};
