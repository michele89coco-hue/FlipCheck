...
openai=async function(body){
 ...
 if(initial&&active164()){
  ...
  body.input[0].content.push({type:'input_text',text:'Se l’oggetto è in una SLAB di grading, trascrivi l’INTERA etichetta in slab_reading.label_text. Estrai soltanto valori letteralmente leggibili: grader, subject, family (serie completa, non il grado), model, year, card_number, variant, grade, certificate. Stringa vuota per dati assenti. Il seriale esemplare a/b non è il certificato né il numero carta. object_match=matches solo se soggetto/aspetto dell’oggetto contenuto corrispondono all’etichetta: spiega gli indizi in match_details, conflict se contrastano, unclear se non visibili. Per altri oggetti slabbed usa model e year come identità. La slab non cambia in lettura fisica un timbro non visibile: conserva separatamente la dichiarazione. Una custodia senza etichetta di grading non è una slab identificativa. Senza slab restituisci null.'});
