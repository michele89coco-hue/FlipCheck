...
function syncIdentity169(value){
 if(active164())value=V164.withSerial184(value,lastVisionReading||value);
...
renderIdent=function(value){priorRender164(value);document.getElementById('specimenSerial184')?.remove();if(value?.kind==='card'){const number=value.source_confirmed_catalog_number||V164.cardKeyFacts(value)?.number.value,serial=value.physical_serial;if(number||serial){const detail=document.createElement('div');detail.id='specimenSerial184';detail.className='status';detail.textContent=[number?'Numero carta nel set: '+number:'',serial?'Numerazione esemplare: '+serial.value+' · Tiratura: '+serial.print_run:''].filter(Boolean).join(' · ');$('identPanel').appendChild(detail);}}if(!value?.assistance_state)return;const panel=document.createElement('div');panel.className='status';panel.id='visualResult';panel.textContent=value.core_identity?.status==='confirmed'&&!V164.ready(value)?'Identità principale verificata. Variante o stampa ancora da verificare.':value.assistance_message||assistanceMessages164[value.assistance_state]||'Ricerca assistita completata.';
