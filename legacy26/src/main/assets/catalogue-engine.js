...
function comparisonValue222(field,v,domain){
 ...
 if(field==='season')return season(v);
...
function season(v){return str(v).match(/\b(?:19|20)\d{2}(?:\s*[-/]\s*(?:\d{4}|\d{2}))?\b/)?.[0].replace(/\s/g,'').replace('/','-').replace(/^(\d{4})-\d{2}(\d{2})$/,'$1-$2')||'';}
