...
function reduceObject(l,entries,error){
 ...
 l.candidates=candidates;const variants=unique(candidates.filter(e=>e.exact).map(e=>norm([e.family,e.subject,e.number].join(' ')))),exact=variants.length===1,core=exact||!!product&&(l.domain==='sealed'||candidates.length>0),entry=exact?candidates.find(e=>e.exact):candidates[0],family=product||entry?.family||'',model=[year||entry?.year,family,l.domain==='generic'&&!norm(family).includes(norm(brand))?brand:'',exact&&entry?.object_identity_verified?(l.domain==='sealed'?str(entry.entry_quote).match(/\b(?:Hobby|Jumbo|Blaster|Mega|Value) Box\b/i)?.[0]||'':str(entry.entry_quote).match(/(?:cartonato|hardcover)/i)?.[0]?(str(entry.entry_quote).match(/(?:cartonato|hardcover)/i)[0]+(k.colors.includes('green')?' verde':'')):''):''].filter(Boolean).join(' · '),pending=exact?[]:[l.domain==='sealed'?'configuration':'model'];
 const result={...l.base,engine_version:193,engine_final:true,kind:'object',title:model||l.base.title,brand,family,model:core?model:'',variant:exact&&l.domain==='sealed'?str(entry.entry_quote).match(/\b(?:Hobby|Jumbo|Blaster|Mega|Value) Box\b/i)?.[0]||'':'',model_verified:core,model_confidence:null,market_ready:exact,catalogue_core_verified:!!entry,catalogue_verified:exact,visual_reference_id:entry?.visual_reference_id||null,field_proof:entry?.field_proof||null,status:core?'identified':'uncertain',identity_status:core?'confirmed':'partial',exact_identity_status:exact?'confirmed':core?'variant_pending':'unresolved',core_identity:{status:core?'confirmed':'partial',origin:entry?'catalogue_and_photo':'printed_product',model,fields:[{field:'brand',value:l.pick('brand')?.value||'',origin:'photo'},{field:'product',value:family,origin:product?'photo':'catalogue'},{field:'year',value:year,origin:'photo'}].filter(f=>f.value),pending_fields:core?[]:pending},variant_resolution:{status:exact?'confirmed':'pending',pending,labels:exact?[entry.subject]:[],proof:exact&&entry?.visual_reference_id?[{field:'configuration',origin:'image_comparison',reference_id:entry.visual_reference_id,source:entry.source}]:[]},variant_needs_verification:!exact,variant_check:exact?'confirmed':'pending',assistance_state:exact?'confirmed':error||'physical_detail_needed',missing_information:exact?[]:[l.domain==='sealed'?'Formato e configurazione della confezione':'Codice modello'],next_photo_request:exact?null:/fumetto|comic|book|libro/i.test(l.base.category||'')?'Fotografa il colophon o la pagina con editore, anno ed edizione del volume.':photoRequest(pending,l.domain),normalized_query:exact?model:'',verification_summary:exact?'Prodotto e configurazione identificati.':'Prodotto riconosciuto; resta il dettaglio indicato.',identification_sources:candidates.map(e=>e.source),candidate_models:candidates.filter(e=>!e.configuration_conflicts.length).map(e=>({model:e.subject,reason:'Configurazione da verificare'}))};
...
function identityPresentation200(l,result,entry){
 ...
 if(l.domain==='sports'&&entry){const product=l.pick('product')?.value,subset=entry.subset_known===true&&subsetKey(entry.subset)!=='base'?str(entry.subset).replace(/ Checklist$/i,''):null;const family=product&&(norm(product).includes(norm(entry.family))||productEquivalent217(product,entry.family))?str(product).replace(/^(?:19|20)\d{2}(?:[-/]\d{2,4})?\s+/,''):entry.family;c.product=family;c.subset=subset||null;base=[c.date||result.source_confirmed_year,family,subset&&!norm(family).includes(norm(subset))?subset:'',c.number?'#'+number(c.number):'',c.subject].filter(Boolean).join(' · ');}
 ...
 result.identity_display=unique(parts.flatMap(p=>str(p).split(/\s*·\s*/))).filter(Boolean).join(' · ');result.title=result.identity_display;
