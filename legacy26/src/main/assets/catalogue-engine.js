...
function semanticField(domain,field,value){
 const t=norm(value);
 if(domain==='sports'&&field==='model_code'&&numberParts(value))return 'collector_number';
...
function normalizePrinting226(l){
 ...
 for(const a of labels){
  ...
  for(const name of candidates){
   ...
   const surface=surfaces.find(b=>b.id!==a.id&&((' '+norm(b.raw)+' ').includes(' '+norm(name)+' ')||a.field==='text'&&/^[\p{L}]{4,}[™®]?$/u.test(a.value.trim())&&['pattern','finish'].includes(b.field)&&/concentric|circol|circular|reticol|checker|lattice/i.test((b.source_value||b.value)+' '+b.raw)&&!/absent|unclear|not visible/i.test(b.raw)));
...
function pokemonPolicy204(l,entry,result){
 ...
 const reasons=result.reasons.filter(r=>!['different_printing_language','different_product_season',...(p.number_role==='pokedex_number'?['different_number']:[])].includes(r));
 ...
 if(p.language&&entryLanguage&&!printingLanguageCompatible201(p.language,entryLanguage)&&!(entry.reference_core_only===true&&['en','it','fr','de','es','pt'].includes(p.language)&&['en','it','fr','de','es','pt'].includes(entryLanguage)&&direct&&numbersMatch(p.number,entry.number)))reasons.push('different_printing_language');
 if(p.year&&entryYear&&p.year!==entryYear)reasons.push('different_product_season');
 if(p.number&&p.number_role!=='pokedex_number'&&!numbersMatch(p.number,entry.number))reasons.push('different_number');
 ...
 const anchored=!!(p.subject&&p.number&&p.language&&(direct||alias||visual)&&(p.number_role==='pokedex_number'?(visual||entry.identifier_type==='pokedex'&&numbersMatch(p.number,entry.number)):numbersMatch(p.number,entry.number)));
 if(p.number_role==='pokedex_number'&&!visual)score=Math.min(score,70);
 const nonblocking=anchored&&p.complete&&p.number_role!=='pokedex_number'?reasons.filter(r=>secondary.includes(r)):[];
...
// A naked date in the back body is contextual; explicit product labels remain binding.
// Preserve the original observation even when it cannot establish the product season.
function productSeasons234(l){return l.evidence('season').filter(a=>{
 if(l.domain!=='sports')return true;
 if(a.zone==='statistics')return false;
 if(a.zone!=='back')return true;
 const years=season(a.value,l.evidence('copyright').map(a=>a.value).join(' ')).match(/^(\d{4})-(\d{2})$/);if(!years)return true;
 const end=Math.floor(+years[1]/100)*100+(+years[2])+(+years[2]<(+years[1]%100)?100:0);
 const copyrights=l.values('copyright').map(a=>+copyrightYear222(a.value)).filter(Boolean);
 return copyrights.length>0&&Math.max(...copyrights)<=end;
 });}
function keyValues(l){return {pokedex:unique(l.evidence('pokedex_number').map(a=>a.value)),illustrator:l.pick('illustrator')?.value||'',attacks:l.evidence('attack').map(a=>a.value),setCodes:unique(l.evidence('set_code').map(a=>a.value)),subject:l.pick('subject')?.value||'',numbers:unique(l.active('collector_number').filter(a=>a.level==='observed').map(a=>numberParts(a.value)?.full)),year:l.domain==='pokemon'?pokemonKeys204(l).year:(l.domain==='sports'?season(productSeasons234(l)[0]?.value,l.evidence('copyright').map(a=>a.value).join(' ')):season(l.pick('season')?.value,l.evidence('copyright').map(a=>a.value).join(' ')))||'',copyright:unique(l.values('copyright').flatMap(a=>str(a.value).match(/\b(?:19|20)\d{2}\b/g)||[])),language:l.pick('language')?.value||'',brands:unique(l.values('brand').map(a=>a.value)),colors:unique(l.evidence('border_color').map(a=>a.value)),patterns:unique(l.evidence('pattern').flatMap(a=>tokens(a.value,PATTERNS))),serials:unique(l.active('serial').filter(a=>a.level==='observed').map(a=>serial(a.value)?.value)),products:unique(l.evidence('product').filter(a=>semanticField(l.domain,'product',a.value)==='product').concat(l.domain==='generic'?l.evidence('model_code'):[]).map(a=>a.value))};}
...
function pokemonQuery203(l,mode,entry){
 const k=keyValues(l),nums=pokemonNumbers203(l),date=l.catalogueDiscrepancy232?'':k.year||'';
 ...
 const hints=mode==='reference'?[entry?.family,languageName,...k.setCodes,...(k.pokedex.length?[k.illustrator,...k.attacks]:[]),'card catalogue image',entry?'(site:psacard.com OR site:pricecharting.com)':'']: [languageName,...k.setCodes,...(k.pokedex.length?[k.illustrator,...(mode==='recovery'?k.attacks:[])]:[]),mode==='variant'?'set card printing variants':mode==='recovery'?'card catalogue alternate printing':'card set checklist'];
...
function query(l,mode='identity',entry=null){
 ...
 const k=keyValues(l);
 if(l.base.object_unit==='panel')return [k.subject,...l.evidence('text').map(a=>a.value),...l.evidence('layout').map(a=>a.value),...k.colors,'collectible card panel image'].filter(Boolean).join(' ');
...
function evaluate(l,entry){
 ...
 if(l.domain==='sports'&&entry.year&&productSeasons234(l).some(a=>!sportsSeason215(a.value,entry.year)))reasons.push('different_product_season');
 if(l.domain==='sports'&&entry.brand&&l.evidence('brand').some(a=>!norm(a.value).includes(norm(entry.brand))&&!norm(entry.brand).includes(norm(a.value))&&!([norm(a.value),norm(entry.brand)].every(v=>['topps','bowman'].includes(v)))))reasons.push('different_manufacturer');
 ...
 const names=[entry.subject,...(l.domain==='sports'&&!(l.base.object_unit==='panel'&&entry.source_tier==='lens_visual_verified'&&l.evidence('catalogue_core').some(a=>a.value===coreKey(entry)))?[]:list(entry.aliases))],needsSubjectVisual=l.domain==='pokemon'&&/[\u3040-\u9fff]/.test(k.subject)&&!names.some(n=>subjectMatch(k.subject,n));
...
function canonicalEntries(entries){
 // Named parallels qualify a subset; they do not create a different card core.
 entries=entries.map(e=>{let subset=str(e.subset);for(const v of list(e.variants)){if(v.name&&norm(subset).endsWith(' '+norm(v.name)))subset=subset.slice(0,subset.length-str(v.name).length).trim();}return subset!==e.subset?{...e,reported_subset:e.subset,subset}:e;});
...
function variantState(l,group){
 ...
 if(l.domain==='sports'&&(l.evidence('catalogue_core').some(a=>a.value===coreKey(e))||e.grounded&&sourceTrusted(e.source?.url,l.domain)&&l.pick('collector_number')&&numbersMatch(e.number,l.pick('collector_number').value))){
  ...
  if(label&&surface&&!nonDistinctive&&(e.physical_printing_scope===true||e.source_tier==='lens_visual_verified'||e.grounded&&sourceTrusted(e.source?.url,l.domain)&&label.field==='printing_name'&&/[™®]/u.test(label.raw)&&(!l.active('serial').length||!!sn))&&!options.some(v=>same(v.name,name)&&sn&&v.print_run&&+v.print_run!==sn.print_run))return {status:'confirmed',pending:[],labels:[name],proof:[{field:'variant',value:name,origin:'original_photo',observation:label.id,corroborating_observation:surface.id}],variant_origin:'original_photo',needs_catalogue:false,...(sn?{physical_serial:{...sn,origin:'original_photo',observation:physicalSerial.id}}:{})};
 }
 ...
 if(l.domain==='pokemon'){
  ...
  const recognizedOrdinary=l.domain==='sports'&&e.source_tier==='recognition_api'&&e.provider_rank===0&&sourceTrusted(e.source?.url,l.domain)&&subjectMatch(k.subject,e.subject)&&l.pick('collector_number')&&numbersMatch(l.pick('collector_number').value,e.number)&&k.products.some(p=>productEquivalent217(p,e.family))&&!options.length&&!l.active('serial').length&&!l.active('printing_name').length&&!l.evidence('edition_text').length&&!['autograph','patch'].some(f=>l.pick(f)?.value==='present')&&!k.patterns.some(p=>!['geometric','dots','squares'].includes(p));
  if(recognizedOrdinary)return {status:'confirmed',pending:[],labels:[],proof:[{field:'printing',value:'provider_identity_agrees_with_photo_no_specific_variant_pending',source:e.source}],variant_origin:'recognition_and_photo',needs_catalogue:false};
...
function reduce(l,entries,{error=null}={}){
 ...
 const model=[date&& !str(family).includes(date)?date:'',family,cardNumber?(l.domain==='pokemon'&&(!l.pick('collector_number')&&k.pokedex.length||entry&&['pokedex','unnumbered'].includes(entry.identifier_type))?'No.'+cardNumber:'#'+cardNumber):'',subject].filter(Boolean).join(' · '),pending=variation.pending;
 ...
 const result={engine_version:193,engine_final:true,market_identity:exact?base.market_identity||null:null,kind:base.kind||'card',category:base.category||'Carta',brand:entry?.brand||base.brand||'',family,model:entry?model:'',title:model||base.title||'Oggetto da identificare',variant:variation.labels.join(' · '),language:k.language,condition:base.condition||'',
...
// OCR is a pointer to pixels, never sufficient to confirm a parallel.
function decisiveRequests237(l){
 const requests=[];
 if(l.domain==='sports'&&!l.pick('printing_name')){
  const clue=l.active('ocr_text').find(a=>/^SUPERFRACTOR[™®]?$/i.test(a.value.trim())&&a.region);
  if(clue){
   requests.push({key:'237:printing:'+clue.image_index,field:'text',reason:'printed_parallel_label',image_index:clue.image_index,region:clue.region,readings:[{text:clue.raw,source:'local_ocr'}]});
   const product=l.active('ocr_text').find(a=>a.image_index===clue.image_index&&/\bfrom\s+(?:19|20)\d{2}\s+.+/i.test(a.raw)&&a.region);
   if(product)requests.push({key:'237:product:'+product.image_index,field:'product',reason:'printed_product_line',image_index:product.image_index,region:product.region,readings:[{text:product.raw,source:'local_ocr'}]});
  }
 }
...
function recoveryRequests(l,result=reduce(l,[])){
 ...
 const conflict=/^\d+$/.test(l.pick('collector_number')?.value||'')&&l.domain!=='pokemon'&&result.core_identity?.status!=='confirmed'&&l.pick('collector_number')&&!l.candidates.some(e=>e.eligible&&l.pick('collector_number')&&numbersMatch(e.number,l.pick('collector_number').value))&&l.candidates.some(e=>e.grounded&&subjectMatch(keyValues(l).subject,e.subject)&&e.reasons?.includes('different_number')&&!e.reasons.includes('different_printed_product'));
 if(l.base.object_unit!=='panel'&&result.core_identity?.status!=='confirmed'&&!l.evidence('pokedex_number').length&&(!l.pick('collector_number')||conflict))pending.add('collector_number');
 // Missing copyright is not a reason to spend a read before catalogue discovery.
 // A year conflict can still request an explicit reconciliation later.
 if(!l.deferYearRecovery237&&l.domain==='pokemon'&&result.core_identity?.status!=='confirmed'&&!pokemonKeys204(l).year&&(l.active('copyright').length||l.evidence('pokedex_number').length))pending.add('copyright');
...
function applyDetails(l,details,requests){for(const d of list(details)){
 ...
 const correction=d.evidence_found===true&&(['catalogue_number_conflict','code_preflight','identity_band'].includes(req.reason)&&d.field==='collector_number'||req.reason==='printing_compatibility'&&d.field==='stamp'||req.reason==='shadow_border_verification'&&d.field==='shadow'||req.reason==='catalogue_discrepancy'&&['copyright','illustrator'].includes(d.field));
 if(d.field==='product'&&req.reason==='printed_product_line'&&l.evidence('product').some(a=>!productEquivalent217(a.value,value)&&!norm(value).replace(/ university/g,'').includes(norm(a.value)))){l.record('detail_rejected',{field:d.field,reason:'different_printed_product'});continue;}
 const previous=l.evidence(d.field).filter(a=>!(correction&&a.image_index===req.image_index&&(a.source==='vision'||req.reason==='catalogue_discrepancy'&&a.source==='identity_band')));if(previous.some(a=>!(d.field==='text'||d.field==='product'&&req.reason==='printed_product_line'||(d.field==='collector_number'?numbersMatch(a.value,value):same(a.value,value))))){l.record('conflicting_detail',{field:d.field,value,previous:previous.map(a=>a.id)});continue;}
 l.add(d.field,value,{source:'focused_vision',certainty:'clear',image_index:req.image_index,region:req.region,raw,spatial_evidence:d.field==='shadow'?{illustration_bounds:d.illustration_bounds,right_transition:d.right_transition,bottom_transition:d.bottom_transition,right_description:d.right_description,bottom_description:d.bottom_description}:undefined,request_key:req.key,supersedes:l.values(d.field).filter(a=>a.image_index===req.image_index&&(a.certainty!=='clear'||d.field==='product'&&req.reason==='printed_product_line'||correction&&(a.source==='vision'||req.reason==='catalogue_discrepancy'&&a.source==='identity_band'))).map(a=>a.id)});
 }normalizePrinting226(l);return l;}
...
function applyIdentityBands202(l,packet){
 for(const o of list(packet.observations)){
  ...
  const existing=l.active(o.field).filter(a=>a.image_index===o.image_index),conflict=existing.some(a=>a.certainty==='clear'&&!(['collector_number','pokedex_number'].includes(o.field)?numbersMatch(a.value,value):o.field==='illustrator'?same(str(a.value).replace(/^Illus\.?\s*/i,''),str(value).replace(/^Illus\.?\s*/i,'')):same(a.value,value)));
