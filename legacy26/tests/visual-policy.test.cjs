...
test('recorded Pele comparison closes using the cited description without invented name or lot number',()=>{
 const p=recorded168.pele,out=V.validate(p.vision,p.comparison,p.references);assert.equal(out.market_ready,true);assert.match(out.model,/1958.*37.*Pelé.*Manoel Francisco Santos/i);assert.equal(out.source_confirmed_catalog_number,'');assert.equal(out.catalogue_data.find(f=>f.field==='subject').quote_supported_tokens,true);assert.doesNotMatch(out.model,/Foto de colección sin cortes/);assert.equal(out.authenticity_status,'not_assessed');
});
