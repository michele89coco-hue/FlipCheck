...
test('204: 202 Politoed keeps its Italian keys instead of closing an English printing',()=>{
 ...
 assert.equal(r.market_ready,false);assert.equal(r.physical_card_number,'H23/H32');assert.equal(r.source_confirmed_year,null);assert.equal(r.language,'it');assert.equal(r.printed_year,'2003');
 assert.equal(r.card_identity.has_rarity_symbol,true);assert.match(r.pokemon_printing.copyright_text,/2003/);
 assert.match(E.query(l),/Politoed "H23\/H32" 2003/);assert.ok(!E.recoveryRequests(l,r).some(q=>q.field==='collector_number'));
});
...
test('PSA/PriceCharting reference searches retain the exact candidate, not the species alone',()=>{
 const l=promo(),q=E.query(l,'reference',entry());assert.match(q,/Charizard "006" 1999/);assert.match(q,/Japanese CD Promo/);assert.match(q,/site:psacard.com OR site:pricecharting.com/);
...
test('production core comparison uses an attributed secondary image and ignores slab furniture',async()=>{
 ...
 assert.equal(calls.length,1);assert.match(calls[0].input[0].content.map(c=>c.text||'').join(' '),/carta dentro la custodia/);assert.equal(l.pick('catalogue_core').reference_source,ref.source.url);assert.equal(E.reduce(l,[e,ref]).market_ready,true);
});
