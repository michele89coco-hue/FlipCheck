...
let catalogueFieldsReply={entry_scope:'unknown',fields:[]},cardKeysReply={entries:[]},specificationReply={entries:[]};
...
test('179 repairs wrongly scoped publication fields from existing text with no new image comparison',async()=>{
 ...
 catalogueFieldsReply={entry_scope:'exact_entry',fields};
 const out=await page.evaluate(async fields=>{
  scan164=newContext164();lastVisionReading={kind:'object',category:'collectible portrait panel',object_unit:'panel',variant:'two portraits',variant_scope:'physical_description',market_ready:false,model_confidence:20,photo_clues:[{text:'Alex',role:'text',certainty:'clear',image_index:1}]};
  const ref={id:'ref1',url:'https://catalog.example/panel',title:'Northern Record, issue 17, 1955',text:'Northern Record, issue 17, 1955. Alex and Morgan.',image_data:images.find(Boolean)};
  const c={unit:'panel',decision:'match',identity_level:'exact',variant_status:'not_applicable',same_unit:true,physical_ambiguity:false,conflicts:[],matches:[{reference_id:'ref1',feature:'layout',photo_detail:'two portraits with divider',reference_detail:'two portraits with divider',agrees:true,reference_evidence:'image'},{reference_id:'ref1',feature:'subject',photo_detail:'Alex and Morgan',reference_detail:'Alex and Morgan',agrees:true,reference_evidence:'image'}],fields:fields.filter(f=>['year','issue_number'].includes(f.field)).map(f=>({...f,scope:'listing'}))};
  const reply={candidates:[c],detail_needed_from:'none',physical_detail_needed:null};scan164.lastComparison={reply,references:[ref]};scan164.comparisonHistory=[scan164.lastComparison];
  const base=FlipCheckVisual.validate(lastVisionReading,reply,[ref]);const result=await finishComparison173(base,scan164);return {result,repair:scan164.fieldRepair,budget:scan164.budget.spent()};
 },fields);
 assert.equal(out.result.market_ready,true);assert.match(out.result.model,/Northern Record.*17/);assert.equal(out.repair.state,'completed');assert.equal(requests.length,1);assert.equal(requests[0].text.format.name,'flipcheck_catalogue_fields');assert.equal(requests[0].tools,undefined);assert.doesNotMatch(JSON.stringify(requests[0].input),/input_image/);assert.equal(googleRequests.length,0);assert.ok(out.budget<=.03);
});
test('179 printing ambiguity routes to original crops and closes only after a clear physical reread',async()=>{
 await reset();await upload();const d=require('./fixtures/identity-cases.cjs').machamp;
 printingReply={...d.vision.pokemon_printing,artwork_shadow:'absent',shadow_image:1,shadow_location:'Clear right and lower artwork border',copyright_image:1,copyright_text:'©1995, 96, 98, 99 Nintendo ©1999 Wizards'};
 const out=await page.evaluate(async d=>{
  scan164=newContext164();lastVisionReading=structuredClone(d.vision);const refs=d.references.map(r=>({...r,image_data:images.find(Boolean)}));
  const candidates=d.candidates.map(c=>({...c,decision:'possible',physical_ambiguity:true,ambiguity_scope:'variant',identity_level:'exact'}));
  const reply={candidates,detail_needed_from:'target',physical_detail_needed:'Read the right and lower artwork border for the shadow.'};
  const base=FlipCheckVisual.validate(lastVisionReading,reply,refs),result=await finishIdentity171(base,scan164);return {base,result,printing:scan164.printingRecovery};
 },d);
 assert.equal(out.base.catalogue_core_verified,true);assert.equal(out.printing.attempted,true);assert.equal(out.result.market_ready,true);assert.match(out.result.variant,/Shadowless/);assert.equal(out.result.identity_basis.variant,'physical_evidence');assert.deepEqual(requests.map(r=>r.text.format.name),['flipcheck_printing_detail']);assert.equal(googleRequests.length,0);
});
test('179 resolver cannot overwrite an observed unresolved green parallel with Base',async()=>{
 await reset();const d=require('./fixtures/diagnostics-170.json').doncic;await upload();
 const out=await page.evaluate(d=>{
  scan164=newContext164();lastVisionReading=d.vision;
  const refined={candidate_checks:[{model:d.vision.model,brand:d.vision.brand,family:d.vision.family,variant:'Base Prizm',source_count:3,matched_terms:d.vision.photo_clues.map(c=>c.text),missing_terms:[],conflicting_terms:[],source_specificity:'exact_model'}]};
  return mergeResolvedFingerprint(d.vision,refined,[],buildFingerprintSignature(d.vision,''));
 },d);
 assert.equal(out.variant,d.vision.variant);assert.equal(out.variant_hypotheses[0].value,'Base Prizm');assert.equal(out.variant_hypotheses[0].origin,'web_hypothesis');assert.equal(out.core_identity.status,'confirmed');assert.equal(out.core_identity.origin,'photo');assert.equal(out.market_ready,false);
});
...
test('optional Google key does not disable typed clues or known identity',async()=>{await reset();await page.evaluate(()=>{$('googleApiKey').value='';});vision=known;await upload();await identify();assert.equal(requests[0].text.format.schema.properties.photo_clues.type,'array');assert.equal(googleRequests.length,0);assert.equal(await page.evaluate(()=>ident.market_ready),true);});
...
test('physical observations and PDF figures are passed to comparison before Google',async()=>{
 ...
 await upload();await identify();assert.deepEqual(requests.map(r=>r.text.format.name),['flipcheck_identification','flipcheck_resolver','flipcheck_visual_comparison']);assert.match(requests[1].input,/physical_observations.*Rectangular body/);assert.equal(googleRequests.length,1);assert.equal(googleRequests[0].action,'page');assert.match(JSON.stringify(requests[2].input),/retrieved_pdf_pages/);assert.match(JSON.stringify(requests[2].input),/pages_rendered/);assert.equal(await page.evaluate(()=>ident.market_ready),true);
});
...
test('185 two-photo keyed sports lookup skips unavailable Google and retains the budget',async()=>{
 ...
 const d=await page.evaluate(()=>diagnostic26());assert.equal(d.uploadedImageCount,2);assert.equal(requests.filter(r=>r.tools?.length).length,2);assert.equal(googleRequests.filter(r=>r.action==='detect').length,0);assert.equal(d.visualAssistance.calls.some(c=>c.provider==='google'),false);assert.equal(d.visualAssistance.continuationBudget.strategy,'fund_next_evidence_request');assert.ok(d.visualAssistance.budget.spentOrReservedUsd<=.03);assert.equal(d.identification.market_ready,false);
});
...
test('unattributed Google partial images are retained and local OCR does not spend API budget',async()=>{
 await reset();await upload();const output=await page.evaluate(async()=>{
  const old=window.FlipCheckGoogle,photo=images.find(Boolean);scan164=newContext164();
  window.FlipCheckGoogle={ocrAvailable(){return true;},request(id,action,payload){if(action!=='image')throw new Error('Unexpected paid/page request');FlipCheckDirect.receive(id,{status:200,image_data:photo});},readText(id){FlipCheckDirect.receive(id,{state:'ok',origin:'on_device_reference_ocr',text:'ACME 2018 ZX-430'});},cancel(){}};
  try{const found=await FlipCheckDirect.references({state:'ok',pagesWithMatchingImages:[],partialMatchingImages:[{url:'https://image.example/label.jpg'},{url:'https://image.example/label.jpg'}]},{});await retainReferences173(found.references,scan164);await retainReferences173(found.references,scan164);return {refs:scan164.retainedReferences,ocr:scan164.localOcr,spent:scan164.budget.spent()};}finally{window.FlipCheckGoogle=old;}
 });
 assert.equal(output.refs.length,1);assert.equal(output.refs[0].discovery_only,true);assert.equal(output.refs[0].text_origin,'unattributed_image');assert.equal(output.refs[0].url,'https://image.example/label.jpg');assert.equal(output.ocr.attempts,1);assert.equal(output.ocr.completed,1);assert.equal(output.spent,0);assert.equal(output.refs[0].image_data,undefined);
});
test('focused photo reading uses an original crop and cannot overwrite an already clear clue',async()=>{
 await reset();await upload();detailReply={details:[{clue_index:0,text:'ZX-430',role:'model',certainty:'clear'},{clue_index:1,text:'Wrong replacement',role:'text',certainty:'clear'}]};
 const out=await page.evaluate(async()=>{scan164=newContext164();lastVisionReading={kind:'object',market_ready:false,photo_clues:[{text:'ZX-43?',role:'model',certainty:'uncertain',image_index:1,region:{image_index:1,x:.2,y:.3,width:.5,height:.1,certain:true}},{text:'ACME',role:'text',certainty:'clear',image_index:1,region:null}]};const result=await rereadPhotoDetails173(lastVisionReading,scan164);return {result,detail:scan164.detailReread,budget:scan164.budget.visionCalls};});
 assert.equal(requests.length,1);assert.equal(requests[0].tools,undefined);assert.equal(out.result.photo_clues[0].text,'ZX-430');assert.equal(out.result.photo_clues[1].text,'ACME');assert.equal(out.detail.images[0].cropped,true);assert.equal(out.detail.images[0].originalHeight,600);assert.ok(out.detail.images[0].sentHeight<100);assert.equal(out.budget,1);
});
test('typed card variant gathers Google and text evidence before one combined comparison',async()=>{
 await reset();vision={...structuredClone(unknown),kind:'card',object_unit:'single',variant:'Green, unconfirmed',photo_clues:[{text:'280',role:'collector_number',certainty:'clear',image_index:1,region:null},{text:'Visible player',role:'text',certainty:'clear',image_index:1,region:null}],physical_observations:[{feature:'color',text:'Green frame',certainty:'clear',entity:'target',image_index:1}]};
 resolverMode='catalogue';catalogText+=' Parallel: Green.';catalogTitle=catalogText;
 comparison.candidates[0]={...candidate(),unit:'single',variant:'Green',matches:[...candidate().matches,{reference_id:'ref1',feature:'code',photo_detail:'280',reference_detail:'#280',reference_evidence:'image',agrees:true},{reference_id:'ref1',feature:'color',photo_detail:'Green frame',reference_detail:'Green frame',reference_evidence:'image',agrees:true}],fields:[...candidate().fields,{field:'variant',value:'Green',reference_id:'ref1',quote:'Parallel: Green.',scope:'target',number_kind:'none',evidence:'text'}]};
 await upload();await identify();const d=await page.evaluate(()=>diagnostic26());assert.deepEqual(requests.map(r=>r.text.format.name),['flipcheck_identification','flipcheck_resolver','flipcheck_visual_comparison']);assert.equal(googleRequests.filter(r=>r.action==='detect').length,1);assert.equal(d.identification.market_ready,true);assert.equal(d.visualAssistance.budget.visionCalls,2);assert.equal(d.visualAssistance.retainedReferences.length,1);
});
test('an empty completed web search permits one distinct short query and never a third',async()=>{
 await reset();vision={...structuredClone(unknown),kind:'card',object_unit:'panel',photo_clues:[{text:'FIRST LONG NAME',role:'text',certainty:'clear',image_index:1,region:null},{text:'SECOND LONG NAME',role:'text',certainty:'clear',image_index:1,region:null},{text:'THIRD',role:'text',certainty:'clear',image_index:1,region:null}]};providerMode='no_images';resolverMode='empty_complete';
 await upload();await identify();const web=requests.filter(r=>r.tools?.length),d=await page.evaluate(()=>diagnostic26());assert.equal(web.length,2);assert.notEqual(web[0].input.split('\n')[0],web[1].input.split('\n')[0]);assert.match(web[1].input.split('\n')[0],/FIRST LONG NAME THIRD/);assert.equal(d.visualAssistance.secondQuery.state,'completed');assert.ok(d.visualAssistance.budget.spentOrReservedUsd<=.03);assert.equal(d.identification.market_ready,false);
});
test('a missing cited code can use one focused comparison without another web or Google request',async()=>{
 await reset();await upload();comparison.candidates[0].matches.push({reference_id:'ref1',feature:'code',photo_detail:'ZX-430',reference_detail:'ZX-430',reference_evidence:'image',agrees:true});
...
test('a card image repair preserves partial keys and cannot invent a missing release year',async()=>{
 await reset();const d=structuredClone(require('./fixtures/identity-cases.cjs').machamp);
 vision={...structuredClone(unknown),...d.vision,market_ready:true};resolverMode='catalogue_identity';catalogText=d.references[0].text.replaceAll('Shadowed','Shadowless');catalogTitle=catalogText;
 ...
 assert.deepEqual(requests.map(r=>r.text.format.name),['flipcheck_identification','flipcheck_resolver','flipcheck_visual_comparison','flipcheck_visual_comparison']);
 assert.equal(googleRequests.filter(r=>r.action==='detect').length,0);
 assert.equal(out.visualAssistance.focusedReview.reason,'insufficient_visual_comparison');
 assert.equal(out.identification.core_identity.status,'partial');assert.ok(out.identification.core_identity.pending_fields.includes('year'));
 assert.equal(out.identification.source_confirmed_catalog_number,'9/102');assert.equal(out.identification.market_ready,false);
 assert.equal(out.visualAssistance.printingRecovery,undefined);assert.ok(out.visualAssistance.budget.visionCalls<=4);assert.ok(out.visualAssistance.budget.spentOrReservedUsd<=.03);
});
...
test('184 a box closes from cited configuration without comparing unrelated single-card images',async()=>{
 await reset();await upload();resolverMode='catalogue_identity';providerMode='placeholder';catalogTitle='2031-32 Example Chrome Updates Basketball Box Guide';
 const quote='A Hobby Box contains 20 packs with four cards per pack and guarantees one autograph.';catalogText=catalogTitle+'\n'+quote+'\nA Mega Box contains six packs.';
 specificationReply={entries:[{reference_id:'page1',unit:'box',scope:'configuration',variant:'Hobby',section_quote:quote,variant_quote:''}]};
 const out=await page.evaluate(async()=>{
...
test('184 a footer serial is reread, catalogue row verified, parallel closed and specimen number rendered',async()=>{
 ...
 const quote='Base Terrace Parallels\nGreen /5\nGreen Ice\nGold /10';catalogText=catalogTitle+'\nBase Terrace Checklist\n21 Alex Rivera, Elsewhere\n'+quote;
 detailReply={details:[{clue_index:5,text:'2/5',role:'serial',certainty:'clear'}]};
 const field=(field,value,number_kind='none',quote=value)=>({field,value,quote,number_kind,reference_id:'page1',scope:'target',evidence:'text'});
 cardKeysReply={entries:[{scope:'exact_entry',fields:[field('family',"Select Road to World Cup '32 Soccer"),field('subject','Alex Rivera','none','21 Alex Rivera, Elsewhere'),field('catalog_number','21','card_number','21 Alex Rivera, Elsewhere'),field('year','2031-32','season')]}]};
 ...
 const out=await page.evaluate(async()=>{
  scan164=newContext164();lastVisionReading={kind:'card',object_unit:'single',category:'soccer trading card',brand:'Example',family:'Select Road to World Cup 2032',model_confidence:94,market_ready:false,variant:'Green parallel unconfirmed',variant_scope:'commercial',identity_basis:{family:'printed',variant:'physical_evidence'},photo_clues:[['Alex Rivera','subject'],['No. 21','collector_number'],['TERRACE','edition'],['ROAD TO WORLD CUP 2032','season'],['2031-32 Example Select Road to World Cup 2032 Soccer','copyright']].map(([text,role])=>({text,role,certainty:'clear',image_index:1,region:role==='collector_number'?{image_index:1,x:.4,y:.1,width:.15,height:.03,certain:true}:null})),physical_observations:[{feature:'color',text:'Green holographic border',entity:'target',certainty:'clear',image_index:1}]};
  scan164.photoOcr=[{image_index:1,state:'ok',meta:{originalWidth:400,originalHeight:600,rect:{x:0,y:0,width:400,height:600}},lines:[{text:'215',x:.45,y:.9,width:.1,height:.02}]}];
  const result=await resolveIdentificationCheap(lastVisionReading,'');renderIdent(result);return {result,detail:scan164.detailReread,specification:scan164.specificationVerification};
 });
...
test('184 a matched catalogue release date disables an inapplicable stamp despite a different copyright reading',async()=>{
 await reset();await upload();const out=await page.evaluate(async()=>{
  scan164=newContext164();lastVisionReading={kind:'card',object_unit:'single',family:'Summit',model:'Summit Rivermon H7/H32',model_confidence:95,variant:'Holo',variant_scope:'physical_description',physical_observations:[{feature:'finish',text:'Holographic artwork',entity:'target',certainty:'clear',image_index:1}],pokemon_printing:{is_pokemon:true,language:'Italian',set_name:'Summit',card_type:'pokemon',first_edition_stamp:'unclear',stamp_image:1,stamp_location:'Below artwork',stamp_text:'',artwork_shadow:'not_applicable',shadow_image:1,shadow_edges:{right:'not_applicable',lower:'not_applicable'},copyright_text:'©2001 Example',copyright_image:1,slab_text:'',slab_image:0}};
  scan164.textReferences=[{id:'page2',url:'https://catalog.example/rivermon',title:'Rivermon Summit H7',text:'Date de sortie de la carte\n12 mai 2003\nH7/H32',text_origin:'retrieved_page'}];
  const value={...lastVisionReading,catalogue_core_verified:true,core_identity:{status:'confirmed',model:lastVisionReading.model,fields:[{field:'family',value:'Summit'},{field:'subject',value:'Rivermon'},{field:'catalog_number',value:'H7/H32'}]},visual_candidates:[{core_accepted:false,ambiguity_scope:'none',identity_conflicts:[],blocking_fields:['catalogue_not_cited']}],identity_basis:{family:'catalogue',variant:'physical_evidence'},unresolved_identity_fields:['variant']};
  return finishIdentity171(value,scan164);
 });
 assert.equal(out.market_ready,true,JSON.stringify(out));assert.equal(out.printing_check.stamp,'not_applicable');assert.equal(out.pokemon_printing.copyright_text,'©2001 Example');assert.equal(out.source_confirmed_year,'2003');assert.equal(requests.length,0);
});
test('184 image indexes in the initial schema cannot exceed the actual uploaded image count',async()=>{
 await reset();vision=structuredClone(known);await upload();await identify();const schema=requests[0].text.format.schema;
 assert.equal(schema.properties.photo_clues.items.properties.image_index.maximum,1);assert.equal(schema.properties.object_regions.items.properties.image_index.maximum,1);assert.equal(schema.properties.pokemon_printing.properties.stamp_image.maximum,1);
});
