...
test('184 a matched catalogue release date disables an inapplicable stamp despite a different copyright reading',async()=>{
 await reset();await upload();const out=await page.evaluate(async()=>{
  ...
  const value={...lastVisionReading,catalogue_core_verified:true,core_identity:{status:'confirmed',model:lastVisionReading.model,fields:[{field:'family',value:'Summit'},{field:'subject',value:'Rivermon'},{field:'catalog_number',value:'H7/H32'}]},visual_candidates:[{core_accepted:true,ambiguity_scope:'variant',identity_conflicts:[],blocking_fields:['physical_ambiguity','appearance_not_matched']}],identity_basis:{family:'catalogue',variant:'physical_evidence'},unresolved_identity_fields:['variant']};
