...
let server,browser,page,origin,photos,requests=[],googleRequests=[],vision,initialMode='',comparison,comparisonQueue=[],catalogText='',catalogTitle='',printingReply,providerMode='ok',resolverMode='',waitApi=null,errors=[],usageOverrides={};
...
async function reset(enabled=true){usageOverrides={};requests=[];googleRequests=[];initialMode='';vision=structuredClone(unknown);comparison={physical_detail_needed:null,candidates:[candidate()]};providerMode='ok';resolverMode='';printingReply=null;comparisonQueue=[];catalogText='Catalogue entry: Documented model.';catalogTitle=catalogText;waitApi=null;await page.goto(origin);await page.waitForFunction(()=>typeof newContext164==='function');await page.evaluate(enabled=>{window.FlipCheckTestMode='mock';trial={free:true,attempts:2,credits:0};saveTrial();$('apiKey').value='fake-openai';$('visualEnabled').checked=enabled;$('googleApiKey').value='fake-google-key-1234567890';$('scanBudget').value='.03';$('budgetFx').value='1';},enabled);}
