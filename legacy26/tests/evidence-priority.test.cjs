...
test('a mocked original printing reread closes the verified core only when its required details are clear',async()=>{
 ...
 for(const shadow of ['absent','unclear']){
  ...
  Object.assign(env,{FlipCheckEditions:E,localPrinting192:async x=>x,saveEvidence192:()=>{},status:()=>{},guard164:()=>{},guard164AfterError:()=>{},addUsage:()=>{},recordClosure164:()=>{},syncIdentity169:x=>x,validImageCount:()=>1,visualPhoto164:async()=>({data:'synthetic crop',meta:{imageIndex:1,cropped:true}}),parseResponseJSON:x=>x,
