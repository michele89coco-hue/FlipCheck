...
test('detail confirmation reaches final identity, diagnostics and visible stamp text together',async()=>{
 for(const [value,label] of [['present','Timbro 1st Edition visibile'],['absent','Timbro 1st Edition assente']]){
  ...
  const out=await scan(),r=out.identification;assert.equal(r.printing_check?.stamp,'confirmed',JSON.stringify(out.visualAssistance.engine));assert.equal(r.printing_check.stamp_presence,value);assert.equal(r.pokemon_printing.first_edition_stamp,value);assert.equal(r.job_status,'variant_resolved');assert.equal(out.visualAssistance.jobStatus,r.job_status);assert.ok(out.visualAssistance.detailReread.committed_revision<=out.visualAssistance.stateRevision);assert.equal(r.next_photo_request,null);assert.match(await page.locator('#printingPanel').innerText(),new RegExp(label));assert.equal(await page.locator('#marketBtn').isDisabled(),false);assert.deepEqual(stages(),['flipcheck_identification','flipcheck_catalogue_search','flipcheck_evidence_detail']);
 }
});
test('affirmative original-photo text resolves an unclear stamp before another detail call',async()=>{
 await reset('politoed',{packet:stampPacket('Stamp 1st Edition visibile sotto l’illustrazione'),noOcr:true,mutate:stampPages});const out=await scan();assert.equal(out.identification.printing_check?.stamp,'confirmed');assert.equal(out.identification.market_ready,true);assert.ok(!stages().includes('flipcheck_evidence_detail'));assert.match(await page.locator('#printingPanel').innerText(),/Timbro 1st Edition visibile/);
});
...
test('193 new observation schema reaches the final UI, without legacy identity fields',async()=>{
 ...
 await reset('politoed',{packet,tcgdex:true});const out=await scan();assert.equal(out.identification.family,'Skyridge');assert.equal(out.identification.market_ready,true,JSON.stringify(out.visualAssistance.engine));assert.equal(out.identification.language,'it');assert.ok(api[0].text.format.schema.required.includes('observations'));assert.ok(!api[0].text.format.schema.required.includes('model'));assert.deepEqual(stages(),['flipcheck_identification']);
});

test('193 official PSA certificate closes identity with the stored grade and no later AI',async()=>{await reset('psa',{certificate:[['Cert Number','64613920'],['Year','1999'],['Set Name','P.M. Japanese CD Promo'],['Card Name','Charizard'],['Card Number','6'],['Variation','Holo'],['Grade','MINT 9']].map(([label,value])=>({label,value}))});const out=await scan();assert.equal(out.visualAssistance.route,'slab_certificate');assert.equal(out.identification.slab_verification.certificate_verified,true);assert.equal(out.identification.market_ready,true);assert.deepEqual(stages(),['flipcheck_identification']);assert.equal(native.length,1);});
