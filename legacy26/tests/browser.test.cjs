...
before(async()=>{
  server=http.createServer((req,res)=>{
    ...
    if(!['index.html','editions.js','targeted-fixes.js','photo-detail.js','identity-web.js','detail-runtime.js'].includes(name)){res.writeHead(404);res.end();return;}
...
test('sports result uses two originals then a bounded web verification',async()=>{
  await reset();await upload(photos.slice(0,2));await page.locator('#identifyBtn').click();await page.waitForFunction(()=>!apiBusy);
  assert.equal(requests.length,2);assert.equal(requests[0].model,'gpt-5.6-luna');assert.equal(requests[0].tools,undefined);
  ...
  assert.equal(await page.evaluate(()=>ident.model_confidence),92);assert.equal(await page.locator('#marketBtn').isDisabled(),false);
...
test('same Vision response displays stamp and Shadowless with matching market query',async()=>{
  ...
  assert.equal(requests.length,2);assert.ok(requests[0].text.format.schema.required.includes('pokemon_printing'));
...
test('uncertain edition keeps identity visible, disables automatic comps and exports without key/images',async()=>{
  ...
  await page.locator('#identifyBtn').click();await page.waitForFunction(()=>!apiBusy);
  assert.equal(requests.length,2);assert.match(await page.locator('#identTitle').textContent(),/Alakazam #1\/102/);
...
test('2560px originals produce precise native-pixel text crops before web; requests and costs counted once',async()=>{
 ...
 response={...kobe,detail_regions:[{image_index:1,purpose:'product_label',x:.65,y:.72,width:.30,height:.20}]};
 apiHandler=(request,n)=>{
  if(!request.tools)return envelope({...response,detail_regions:n===1?response.detail_regions:[],identifier_observations:[]});
...
test('a second web request is distinct and stops after resolving the first empty lookup',async()=>{
 await reset();await upload([photos[0]]);let webs=0;
 apiHandler=request=>{
  if(!request.tools)return defaultEnvelope(request);
  if(++webs===1)return envelope({summary:'No exact source',candidates:[],missing_information:['Exact number'],next_photo_request:null},true,[]);
  return defaultEnvelope(request);
 };
 await page.locator('#identifyBtn').click();await page.waitForFunction(()=>!apiBusy);
...
test('two failed web lookups cannot loop or leave a ready result from the previous object',async()=>{
 await reset();await upload([photos[0]]);apiHandler=request=>!request.tools?defaultEnvelope(request):envelope({summary:'Not found',candidates:[],missing_information:['Code'],next_photo_request:'Read label'},true,[]);
 await page.locator('#identifyBtn').click();await page.waitForFunction(()=>!apiBusy);
 assert.equal(requests.length,3);assert.equal(await page.evaluate(()=>ident.market_ready),false);
...
test('invalid or oversized regions are skipped and small images are never enlarged',async()=>{
 await reset();await upload([photos[0]]);
 response={...kobe,detail_regions:[{image_index:1,purpose:'printed_text',x:0,y:0,width:1,height:1},{image_index:3,purpose:'slab_label',x:0,y:0,width:.5,height:.2}]};
 await page.locator('#identifyBtn').click();await page.waitForFunction(()=>!apiBusy);
