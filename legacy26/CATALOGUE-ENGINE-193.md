...
Le sportive usano la checklist del prodotto. Nome e numero senza altri indizi identificano candidati: il confronto con una foto attribuita al catalogo può distinguere il prodotto anche quando il retro non è disponibile. Parole del nome lette separatamente vengono ricomposte solo se allineate nella stessa zona della foto. Colore del bordo, pattern, finitura, autografo, patch e seriale restano distinti. Un seriale conserva numeratore e tiratura. Le immagini delle varianti devono essere attribuite alla fonte: la somiglianza locale non chiude da sola un'identità.
...
La fixture `diagnostics-192.json.gz` contiene le letture iniziali e le risposte OCR/fonti dei sei test della build 192, oltre al caso Jordan della build 191 forniti dall'utente, selezionate senza credenziali o payload immagine. I replay non effettuano chiamate API a pagamento. Le immagini sintetiche nei test browser sono supporti per verificare trasporto, ritagli e logica: non misurano l'accuratezza di una nuova lettura Vision sulle fotografie.
